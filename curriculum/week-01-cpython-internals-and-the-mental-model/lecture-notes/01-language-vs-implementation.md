...
```mermaid
flowchart TD
  Spec["Python language spec"] --> CPython["CPython implementation"]
  Spec --> PyPy["PyPy implementation"]
  Spec --> Micro["MicroPython implementation"]
  Spec --> Graal["GraalPy implementation"]
  CPython --> Exe["python executable on your machine"]
```
*Three meanings of Python: one spec, many implementations, one executable you actually run.*
