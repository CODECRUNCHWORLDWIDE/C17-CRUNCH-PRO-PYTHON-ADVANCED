...
```mermaid
flowchart LR
  A["stack empty"] -->|"LOAD_FAST x"| B["x"]
  B -->|"LOAD_CONST 1"| C["x, 1"]
  C -->|"BINARY_OP plus"| D["x plus 1"]
  D -->|"RETURN_VALUE"| E["stack empty, value returned"]
```
*Each opcode either pushes or pops the interpreter stack, one step at a time.*
