...
```mermaid
flowchart TD
  Q["Choosing a build backend"] --> A["New pure Python library"]
  Q --> B["Has C C++ or Fortran extensions"]
  Q --> C["Existing CMake or Meson build"]
  Q --> D["Rust extension via PyO3"]
  Q --> E["Tiny one module utility"]
  Q --> F["Already invested in poetry or pdm"]
  A --> RA["hatchling"]
  B --> RB["setuptools"]
  C --> RC["scikit-build-core or meson-python"]
  D --> RD["maturin"]
  E --> RE["flit-core"]
  F --> RF["stay on poetry-core or pdm-backend"]
```
*Which backend to reach for, based on what the project actually needs.*
