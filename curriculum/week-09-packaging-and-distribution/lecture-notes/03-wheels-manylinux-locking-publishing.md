...
sequenceDiagram
  ...
  GH-->>WF: Short lived identity token
  WF->>PyPI: Present OIDC token
  PyPI->>PyPI: Verify repo workflow and environment claims
  PyPI-->>WF: Mint short lived API token
  WF->>PyPI: Upload wheel and sdist
```
*Trusted publishing swaps a stored secret for a short-lived OIDC handshake.*
...
```mermaid
flowchart LR
  A["Push tag v star"] --> B["Job build: python -m build"]
  B --> C["Upload dist artifact"]
  C --> D["Job publish-testpypi via OIDC"]
  D --> E["Job publish-pypi via OIDC"]
```
*The three-job release pipeline: a tag push flows through build, TestPyPI, then PyPI.*
