...
### Common student mistakes (when adapting for the mini-project)
...
- **TestPyPI's token from `~/.pypirc` does not match PyPI's.** They are separate accounts with separate tokens. Generate one for each. Some students attempt to use a PyPI token to upload to TestPyPI; it fails authentication.
