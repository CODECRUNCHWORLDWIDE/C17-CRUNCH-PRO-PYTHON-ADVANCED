...
```mermaid
flowchart LR
    A["pyx source"] --> B["Cython compiler"]
    B --> C["c file, CPython C API"]
    C --> D["C compiler"]
    D --> E["so extension module"]
    E --> F["import in Python"]
```
*The Cython pipeline turns typed Python-like source into a compiled extension module.*
...
```mermaid
flowchart TD
    A{"Uniform SIMD-tuned op over the whole array"} -->|"Yes"| B["NumPy wins"]
    A -->|"No"| C{"Data-dependent memory access"}
    C -->|"Yes"| D["Cython wins"]
    C -->|"No"| E{"Needs per-element short circuit"}
    E -->|"Yes"| D
    E -->|"No"| B
```
*Whether NumPy or a hand-written Cython kernel wins depends on the shape of the per-element work.*
...
A common learner finding: "I wrote my kernel in ctypes, cffi API, and Cython. They all run in the same time. Why?"
...
Three things that bite learners:
