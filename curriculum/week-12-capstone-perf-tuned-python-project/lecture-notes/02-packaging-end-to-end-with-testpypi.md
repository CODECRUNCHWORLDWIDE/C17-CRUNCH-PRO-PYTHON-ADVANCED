...
```mermaid
flowchart LR
  A["Source layout pyproject toml"] --> B["python dash m build"]
  B --> C["sdist and wheel in dist"]
  C --> D["twine check dist"]
  D --> E["twine upload testpypi"]
  E --> F["Clean venv pip install"]
  F --> G["Smoke test import"]
```
*The end-to-end packaging pipeline, from source layout to a verified install on a clean machine.*
