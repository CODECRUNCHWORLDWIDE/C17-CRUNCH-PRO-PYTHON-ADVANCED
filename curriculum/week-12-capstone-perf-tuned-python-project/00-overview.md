...
| File | What's inside |
|------|---------------|
| [README.md](./00-overview.md) | This overview; the W1–W12 wrap-up sidebar |
| [resources.md](./01-resources.md) | The PyPA documentation index, TestPyPI guides, every PEP cited from W1 through W12, profiling tools |
| [lecture-notes/01-capstone-scoping-and-the-tier-ladder.md](./02-lecture-notes/01-capstone-scoping-and-the-tier-ladder.md) | How to scope a perf project; the tier ladder; the discipline of skipping tiers |
| [lecture-notes/02-packaging-end-to-end-with-testpypi.md](./02-lecture-notes/02-packaging-end-to-end-with-testpypi.md) | The full lifecycle: `pyproject.toml` → `build` → `twine upload --repository testpypi` → `pip install` verification |
| [lecture-notes/03-the-benchmark-report-as-deliverable.md](./02-lecture-notes/03-the-benchmark-report-as-deliverable.md) | How to write a benchmark report a reviewer can reproduce |
| [exercises/exercise-01-naive-baseline-and-profile.py](./03-exercises/exercise-01-naive-baseline-and-profile.py) | Write the naive baseline; profile it; identify the hot path |
| [exercises/exercise-02-tier-ladder-walkthrough.py](./03-exercises/exercise-02-tier-ladder-walkthrough.py) | Apply the tier ladder to a 1D blur kernel; measure each tier |
| [exercises/exercise-03-packaging-skeleton.py](./03-exercises/exercise-03-packaging-skeleton.py) | Generate the `pyproject.toml`, `src/` layout, `py.typed`, and verify with `python -m build` |
| [exercises/exercise-04-testpypi-dry-run.py](./03-exercises/exercise-04-testpypi-dry-run.py) | Dry-run the TestPyPI upload locally; lint the metadata; verify the wheel contents |
| [exercises/SOLUTIONS.md](./03-exercises/SOLUTIONS.md) | Expected outputs, common errors, the reasoning behind each exercise |
| [challenges/challenge-01-end-to-end-rehearsal.md](./04-challenges/challenge-01-end-to-end-rehearsal.md) | One-day rehearsal: take a 50-line script through the full pipeline before committing to your capstone kernel |
| [challenges/challenge-02-reproducibility-audit.md](./04-challenges/challenge-02-reproducibility-audit.md) | Hand your benchmark to a peer and watch them try to reproduce it; fix every gap in the methodology |
| [quiz.md](./05-quiz.md) | Final exam — 20 questions covering W1–W12 |
