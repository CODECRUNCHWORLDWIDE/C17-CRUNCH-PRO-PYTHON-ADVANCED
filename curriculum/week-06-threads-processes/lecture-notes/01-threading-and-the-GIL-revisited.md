...
```mermaid
flowchart TD
  A["Dominant operation in your code"] --> B{"Is the GIL held during it"}
  B -- "Yes, pure Python or non releasing C ext" --> C["Threading gives no speedup"]
  B -- "No, GIL released during the call" --> D["Threading gives speedup up to N cores"]
  C --> E["Use ProcessPoolExecutor or 3.13t threads"]
  D --> F["Use ThreadPoolExecutor"]
```
*The GIL-release test: one question decides whether threading helps.*
...
sequenceDiagram
  ...
  Caller->>Executor: submit fn args
  Executor->>Future: create Future
  Executor->>Queue: put WorkItem
  Worker->>Queue: get WorkItem
  Worker->>Worker: run fn args
  Worker->>Future: set_result or set_exception
  Caller->>Future: result
  Future-->>Caller: return value
```
*One submit and result round trip through the executor, queue, and worker thread.*
