...
```mermaid
stateDiagram-v2
  [*] --> Owned : object created by a thread
  Owned --> Owned : refcount update by owning thread, non atomic
  Owned --> Shared : touched by another thread
  Shared --> Shared : refcount update, atomic
  Shared --> [*] : refcount reaches zero
```
*An object's refcount starts cheap and owned, then pays an atomic tax once shared.*
...
```mermaid
flowchart TD
  A["Import a C extension on 3.13t"] --> B{"Py_mod_gil flag"}
  B -- "Py_MOD_GIL_NOT_USED" --> C["Extension claims free threaded compatibility"]
  C --> D["GIL stays disabled"]
  B -- "unset or Py_MOD_GIL_USED" --> E["Interpreter re-enables the GIL"]
  E --> F["Warning printed, threads serialize again"]
```
*What happens to the GIL when a C extension is imported on the free-threaded build.*
