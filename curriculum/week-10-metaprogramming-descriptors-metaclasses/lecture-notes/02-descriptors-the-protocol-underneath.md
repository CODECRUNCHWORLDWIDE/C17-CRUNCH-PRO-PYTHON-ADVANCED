...
```mermaid
flowchart TD
  A["Look up attr in the MRO"] --> B{"Is it a data descriptor"}
  B -->|"yes"| C["Call get and return"]
  B -->|"no"| D["Look up attr in instance dict"]
  D --> E{"Found in instance dict"}
  E -->|"yes"| F["Return the instance value"]
  E -->|"no"| G{"Is it a non data descriptor"}
  G -->|"yes"| H["Call get and return"]
  G -->|"no"| I["Return class attribute or raise AttributeError"]
```
*The four-step precedence walk Python performs for every `instance.attr` lookup.*
