...
```mermaid
flowchart TD
  P["Parent process"] --> S{"Start method"}
  S -- fork --> C1["Copy address space copy on write, microseconds"]
  S -- spawn --> C2["Launch fresh interpreter, reimport modules, 50 to 300 ms"]
  S -- forkserver --> C3["Clean single threaded fork server forks each worker"]
```
*Three ways a worker process comes into being, trading startup speed against clean state.*
...
sequenceDiagram
  ...
  participant Pipe
  participant Worker
  Parent->>Parent: pickle the chunk
  Parent->>Pipe: write pickled chunk
  Pipe->>Worker: read pickled chunk
  Worker->>Worker: unpickle args
  Worker->>Worker: run function
  Worker->>Worker: pickle result
  Worker->>Pipe: write pickled result
  Pipe->>Parent: read pickled result
  Parent->>Parent: unpickle result
```
*The ten-step lifecycle of one task crossing the process boundary and back.*
