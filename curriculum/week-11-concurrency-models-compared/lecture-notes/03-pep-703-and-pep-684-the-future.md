...
```mermaid
mindmap
  root(("PEP 703 removes the GIL"))
    Biased reference counting
    Deferred reference counting
    mimalloc allocator
    Per object locks on dict list set
```
*Four mechanisms replace the single GIL as the guarantor of a consistent heap.*
...
flowchart TD
  subgraph Proc["One OS process"]
    A["Subinterpreter A, own GIL, own modules"]
    B["Subinterpreter B, own GIL, own modules"]
    Q["interpreters Queue, shareable types only"]
    A --> Q
    Q --> B
  end
```
*Subinterpreters share a process but not interpreter state; only shareable types cross the queue.*
