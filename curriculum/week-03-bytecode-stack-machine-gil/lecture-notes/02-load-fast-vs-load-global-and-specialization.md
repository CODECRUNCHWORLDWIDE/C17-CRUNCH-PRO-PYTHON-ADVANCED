...
```mermaid
flowchart TD
  A["Name reference in source"] --> B{"Assigned in this function, no global or nonlocal"}
  B -->|Yes| C["Local scope: LOAD_FAST"]
  B -->|No| D{"Free or cell variable from enclosing function"}
  D -->|Yes| E["LOAD_DEREF"]
  D -->|No| F{"Inside a class body"}
  F -->|Yes| G["LOAD_NAME"]
  F -->|No| H["Global or builtin: LOAD_GLOBAL"]
```
*How the compiler's scope classifier maps a name reference to an opcode, before any code runs.*
...
stateDiagram-v2
  [*] --> Generic
  Generic --> Warming: counter ticks down each call
  Warming --> Specialized: counter reaches zero, types match
  Warming --> Generic: no matching specialization, backoff
  Specialized --> Specialized: guard passes, fast path runs
  Specialized --> Generic: guard fails, deopt rewrite
```
*Lifecycle of one opcode under PEP 659 adaptive specialization, from generic to specialized and back on deopt.*
