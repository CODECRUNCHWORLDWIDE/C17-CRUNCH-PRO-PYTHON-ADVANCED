...
```mermaid
flowchart LR
  a["variable a deleted"] -.-> ListA["list A refcount 1"]
  b["variable b deleted"] -.-> ListB["list B refcount 1"]
  ListA -->|"contains"| ListB
  ListB -->|"contains"| ListA
```
*A and B keep each other alive even after both variable bindings are deleted.*
...
```mermaid
flowchart LR
  New["New tracked object"] --> Gen0["Generation 0 youngest"]
  Gen0 -->|"survives a collection"| Gen1["Generation 1"]
  Gen1 -->|"survives a collection"| Gen2["Generation 2 oldest"]
  Gen0 -->|"700 allocations"| Trigger0["Collection runs"]
  Gen1 -->|"10 Gen 0 collections"| Trigger1["Collection runs"]
  Gen2 -->|"10 Gen 1 collections"| Trigger2["Collection runs"]
```
*Objects promote to older generations as they survive collections; older generations are swept less often.*
