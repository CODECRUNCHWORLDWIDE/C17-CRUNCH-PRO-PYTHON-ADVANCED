...
```mermaid
flowchart LR
  subgraph Regular["Regular instance"]
    RObj["Point object"] --> RDict["dict hash table"]
    RDict --> Rx["x"]
    RDict --> Ry["y"]
  end
  subgraph Slotted["Slotted instance"]
    SObj["Point object"] --> Sx["x slot"]
    SObj --> Sy["y slot"]
  end
```
*A regular instance points to a separate dict for its attributes; a slotted instance stores them inline.*
...
```mermaid
flowchart TD
  A["Reproduce locally"] --> B["tracemalloc snapshot before and after"]
  B --> C{"Culprit line clear"}
  C -->|"No"| D["Switch to memray flamegraph"]
  C -->|"Yes"| E["Find the holder if needed"]
  D --> E
  E --> F["objgraph show_backrefs"]
  F --> G["Fix bound cache weakref or close generator"]
  G --> H["Verify with snapshot comparison"]
```
*The leak-hunting playbook as a decision flow from reproduction to verified fix.*
