...
stateDiagram-v2
    [*] --> CREATED
    CREATED --> ENTERED: aenter schedules the deadline callback
    ENTERED --> EXPIRING: deadline fires and task cancel is called
    ENTERED --> EXITED: block exits before the deadline
    EXPIRING --> EXPIRED: aexit uncancels and raises TimeoutError
    EXITED --> [*]
    EXPIRED --> [*]
```
*The Timeout context manager's state machine, from entry to either a clean exit or a converted TimeoutError.*
...
sequenceDiagram
    participant Caller
    participant Outer as Outer future
    participant Inner as Inner task db commit
    Caller->>Outer: await shield inner
    Outer->>Inner: linked by done callback
    Caller--)Outer: caller is cancelled
    Outer->>Caller: raises CancelledError
    Note over Inner: keeps running unaffected
    Inner->>Inner: commit finishes
    Note over Outer: link already removed, result has nowhere to go
```
*Shield decouples the outer await from the inner task, so caller cancellation cannot interrupt the commit.*
