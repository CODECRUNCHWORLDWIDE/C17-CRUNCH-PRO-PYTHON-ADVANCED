...
```mermaid
sequenceDiagram
    participant P as Parent aexit
    participant B as Child B
    participant A as Child A and C
    B->>B: raises ValueError
    B->>P: on_task_done fires
    P->>A: abort cancels siblings
    P->>P: parent_task.cancel
    P->>P: CancelledError in wait loop
    P->>P: loop again until tasks empty
    P->>P: uncancel absorbs the signal
    P->>P: raise BaseExceptionGroup
```
*How one failing child cancels its siblings and surfaces as an ExceptionGroup.*
...
```mermaid
flowchart TD
    G["BaseExceptionGroup two children failed"]
    G --> V["ValueError members"]
    G --> T["TypeError members"]
    G --> R["RuntimeError members"]
    V --> EV["except star ValueError catches this"]
    T --> ET["except star TypeError catches this"]
    R --> ER["Uncaught residual group propagates"]
```
*How except star splits an ExceptionGroup into per-type groups, leaving anything uncaught to re-raise as a smaller residual group.*
