...
stateDiagram-v2
  ...
  PENDING --> FINISHED: set_result
  PENDING --> FINISHED: set_exception
  FINISHED --> [*]
  CANCELLED --> [*]
```
*The Future state machine: three terminal outcomes, no way back to PENDING.*
...
```mermaid
sequenceDiagram
  participant EvLoop as Loop
  participant Task
  participant Coro as coroutine
  participant Fut as future

  EvLoop->>Task: call_soon schedules first step
  Task->>Coro: send None
  Coro-->>Task: yields the future
  Task->>Fut: add_done_callback wakeup
  Note over EvLoop: polls selector until the timer fires
  EvLoop->>Fut: set_result with the result
  Fut->>Task: wakeup callback runs
  Task->>Coro: send None again
  Coro-->>Task: raises StopIteration
  Task->>EvLoop: task result set, loop stops
```
*The full round trip from await to resumption, condensed from the eighteen-step trace above.*
