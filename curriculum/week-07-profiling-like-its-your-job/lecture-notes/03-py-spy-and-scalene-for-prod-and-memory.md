...
```mermaid
flowchart TD
  A{"What is the symptom"} -->|"Local CPU workload"| B["cProfile then line profiler"]
  A -->|"Production stall, cannot restart"| C["py-spy dump"]
  A -->|"Production, want a flamegraph"| D["py-spy record"]
  A -->|"Python vs C question"| E["scalene"]
  A -->|"Memory leak or pressure"| F["scalene with memory tracking"]
```
*Picking among the four tools by symptom, per the decision table above.*
...
```mermaid
flowchart TD
  A["main"] --> B["process_all"]
  B --> C["process_one"]
  C --> D["compute_signature - the hot leaf at 78 percent of samples"]
```
*The call path a flamegraph click-through reveals, from entry point down to the hot leaf.*
